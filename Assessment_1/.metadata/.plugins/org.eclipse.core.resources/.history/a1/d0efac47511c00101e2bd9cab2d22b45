...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseStructure> handleHttpExceptions(RestClientResponseException e) {
        ResponseStructure response = new ResponseStructure();
        response.setStatus(e.getStatusCode().value());  
        response.setMessage("HTTP Error from the targated Service from A Service: " + e.getMessage());

        log.error("HTTP exception caught: Status - {}, Message - {}", e.getStatusCode().value(), e.getMessage());
